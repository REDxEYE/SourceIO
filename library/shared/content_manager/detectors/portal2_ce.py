...
logger = log_manager.get_logger('Portal2CommunityEditionDetector')


class Portal2CommunityEditionDetector(Source1Detector):
    ...
    def find_game_root(cls, path: TinyPath) -> TinyPath | None:
        portal2_ce_gameinfo = backwalk_file_resolver(path, r'p2ce\p2ce.fgd')
        if portal2_ce_gameinfo is not None:
            return portal2_ce_gameinfo.parent.parent
        return None
    ...
    def scan(cls, path: TinyPath) -> tuple[Collection[ContentProvider] | None, TinyPath | None]:
        ...
        portal2_mod_gi_path = portal2_root / "p2ce/gameinfo.txt"
