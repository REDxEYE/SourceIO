from abc import ABCMeta
from pathlib import Path


from SourceIO.library.shared.content_manager.provider import ContentProvider
from SourceIO.library.shared.content_manager.detectors.content_detector import ContentDetector
from SourceIO.library.shared.content_manager.providers.loose_files import LooseFilesContentProvider
from SourceIO.library.shared.content_manager.providers.source2_content_provider import Gameinfo2ContentProvider
from SourceIO.library.shared.content_manager.providers.vpk_provider import VPKContentProvider
from SourceIO.library.utils.path_utilities import backwalk_file_resolver


class Source2Detector(ContentDetector, metaclass=ABCMeta):

    @classmethod
    def scan(cls, path: Path) -> dict[str, ContentProvider]:
        s2_root = None
        s2_gameinfo = backwalk_file_resolver(path, 'gameinfo.gi')
        if s2_gameinfo is not None:
            s2_root = s2_gameinfo.parent.parent
        if s2_root is None:
            return {}
        content_providers = {}
        cls.recursive_traversal(s2_root, s2_gameinfo.parent.stem, content_providers)
        return content_providers


    @classmethod
    def scan_for_vpk(cls, root_dir: Path, content_providers: dict[str, ContentProvider]):
        for vpk in root_dir.glob('*_dir.vpk'):
            content_providers[f'{root_dir.stem}_{vpk.stem}'] = VPKContentProvider(vpk)

    @classmethod
    def recursive_traversal(cls, game_root: Path, name: str, content_providers: dict[str, ContentProvider]):
        if name in content_providers:
            return
        elif not (game_root / name / 'gameinfo.gi').exists():
            content_providers[name] = LooseFilesContentProvider(game_root / name)
            cls.scan_for_vpk(game_root / name, content_providers)
        else:
            gh_provider = Gameinfo2ContentProvider(game_root / name / 'gameinfo.gi')
            content_providers[name] = gh_provider
            cls.scan_for_vpk(game_root / name, content_providers)
            for game in gh_provider.get_paths():
                game = Path(game)
                if game.name.startswith('|'):
                    continue
                elif game.name.endswith('*'):
                    if not (game_root / game).parent.exists():
                        continue
                    for folder in (game_root / game).parent.iterdir():
                        content_providers[folder.stem] = LooseFilesContentProvider(folder)
                elif game.name.endswith('.vpk'):
                    game = game.with_name(game.stem + '_dir.vpk')
                    if (game_root / game).exists():
                        content_providers[Path(game).stem] = VPKContentProvider(game_root / Path(game))
                else:
                    cls.recursive_traversal(game_root, game.stem, content_providers)
