...
from ..structs.bone import Bone2531
from ..structs.bone_controller import BoneController2531
...
# Fields from Crowbar implementation: https://github.com/ZeqMacaw/Crowbar
...
class MdlV2531(Mdl):
    ...
    def from_buffer(cls, buffer: Buffer):
        ...
        for bone_id in range(header.bone_count):
            bone = Bone2531.from_buffer(buffer, header.version)
            bone.bone_id = bone_id
            bones.append(bone)

        # TODO: This hasn't been tested yet..
        bone_controllers = []
        buffer.seek(header.bone_controller_offset)
        for bone_controller_id in range(header.bone_controller_count):
            bone_controller = BoneController2531.from_buffer(buffer, header.version)
            bone_controller.bone_controller_id = bone_controller_id
        ...
        buffer.seek(header.skin_family_offset)
        for _ in range(header.skin_family_count):
            skin_indices.append(buffer.read_uint16())
        ...
        for _ in range(header.body_part_count):
            body_part = BodyPart.from_buffer(buffer, header.version, header)
