...
from . import vs
from .sfm import SFM, Dag, _get_bone
...
# ==================================================================================================
def GetDagFromNameOrObject(target, bCheckResult=True):
    """ Find the dag with the specified name or simply return the target if it is a dag object """
    targetDag = target

    # If the target was specified as name and not an actual
    # dag object, find the dag object using the name
    if isinstance(target, str):
        targetDag = sfm.FindDag(target)

        # If the check result option is true report a fatal
    # error if the reuturn value is not a valid dag object.
    if bCheckResult:
        if targetDag is None:
            raise Exception("Paramter is not a valid dag node or name of a dag node")

    return targetDag


# ==================================================================================================
def FindFirstDag(dagNameList, bRequired=False):
    ...
    for dagName in dagNameList:
        dag = sfm.FindDag(dagName)
        if dag is not None:
            return dag

    if bRequired:
        errorString = "Unable to find dag of name: "
        for dagName in dagNameList:
            errorString += dagName
            errorString += ", "
...
# ==================================================================================================
def Parent(child, parent, nLogMode=vs.REPARENT_LOGS_OVERWRITE, bMaintainWorldPos=True):
    """ Make the dag node specified as the child a child of the dag node specified as the parent. """

    if child is None:
        return

    childDag = GetDagFromNameOrObject(child)
    parentDag = GetDagFromNameOrObject(parent)
...
# ==================================================================================================
def ParentMaintainWorld(child, parent):
    ...
    Parent(child, parent, vs.REPARENT_LOGS_MAINTAIN_WORLD, True)
    return
...
# ==================================================================================================
def CreateHandleAt(handleName, target, bPosControl=True, bRotControl=True):
    """ Create a rig handle with the position and orientation of the specified node """

    sfm.edit_mode()
    targetDag = GetDagFromNameOrObject(target)
    target = _get_bone(sfm.obj, targetDag, 'EDIT')
...
# ==================================================================================================
def CreateConstrainedHandle(handleName, target, bPosControl=True, bRotControl=True, bCreateControls=True):
    ...
    sfm.edit_mode()
    handle_bone = _get_bone(sfm.obj, handle, 'EDIT')
    target_bone = _get_bone(sfm.obj, targetDag, 'EDIT')
...
# ==================================================================================================
def CreateOffsetHandle(handleName, target, offset, bPosControl=True, bRotControl=True, refSpaceDag=None,
...
# ==================================================================================================
def CreatePointOrientConstraint(target, slave, bCreateControls=True, group=None):
    ...
    targetDag = GetDagFromNameOrObject(target)
    slaveDag = GetDagFromNameOrObject(slave)

    sfm.PushSelection()
    SelectDagList([targetDag, slaveDag])
...
def BuildArmLeg(rigPVTarget, rigEndTarget, bipStart, bipEnd, constrainEnd, group=None):
    ...
    pvTargetDag = GetDagFromNameOrObject(rigPVTarget)
    endTargetDag = GetDagFromNameOrObject(rigEndTarget)
    startBoneDag = GetDagFromNameOrObject(bipStart)
    endBoneDag = GetDagFromNameOrObject(bipEnd)
...
# ==================================================================================================
def SetControlGroupColorByName(groupName, rootGroup, color):
    """ Find the control group with the specified name and set its color """
    group = FindChildControlGroup(rootGroup, groupName, True)
    SetControlGroupColor(group, color)
...
# ==================================================================================================
def CreateChannel(name, type, defaultValue, animSet, shot):
    ...
    AddElementToRig(channel, animSet)
    ...
    channelsClip = GetChannelsClipForAnimSet(animSet, shot)
...
# ==================================================================================================
def CreateControlAndChannel(name, type, value, animSet, shot):
    newControl = animSet.FindOrAddControl(name, False)
    if newControl is not None:
        AddElementToRig(newControl, animSet)
        AddAttributeToElement(newControl, "value", type, value)
        AddAttributeToElement(newControl, "defaultValue", type, value)
        channelName = name + "_channel"
        channel = CreateChannel(channelName, type, value, animSet, shot)
...
# ==================================================================================================
def CreateConnection(name, srcElement, srcAttrName, animSet):
    ...
    AddElementToRig(newConnection, animSet)
...
# ==================================================================================================
def CreateExpression(name, expression, animSet):
    ...
    AddElementToRig(newExpression, animSet)
...
# ==================================================================================================
def CreateRotationConstraint(name, axis, slaveDag, animSet):
    ...
    AddElementToRig(rotationConstraint, animSet)
...
# ==================================================================================================
def CreateControlledValue(controlName, attrName, type, value, animSet, shot):
    # Create the control and attach a value channel to it
    control = CreateControlAndChannel(controlName, type, value, animSet, shot)
    ...
    AddElementToRig(valueElement, animSet)
...
# ==================================================================================================
def AddControlsInGroupToDisplaySet(controlGroup, displaySet):
    ...
    if controlGroup is not None:
        for childGroup in controlGroup.children:
            AddControlsInGroupToDisplaySet(childGroup, displaySet)
...
# ==================================================================================================
def AddControlGroupsToDisplaySet(controlGroupList, displaySet):
    ...
    for groupName in controlGroupList:
        if rootGroup.HasChildGroup(groupName, True):
            ...
            AddControlsInGroupToDisplaySet(controlGroup, displaySet)
        else:
...
# ==================================================================================================
def CreateDisplaySetWithControlGroups(displaySetName, controlGroupList, bHidden=False, bSnap=False):
    ...
    AddControlGroupsToDisplaySet(controlGroupList, newDisplaySet)
...
class SfmUtils(metaclass=SingletonMeta):
    ...
    def FindFirstDag(self, bone_names, required=False):
        sfm.pose_mode()
        for name in bone_names:
            ...
            if dag is not None:
                return dag
        if required:
            raise Exception(f'Failed to find bone ({bone_names})')

    def GetDagFromNameOrObject(self, target, check_result=True):
        dag = target
        if isinstance(target, str):
    ...
    def CreateConstrainedHandle(self, handleName, target, bPosControl=True, bRotControl=True, bCreateControls=True):
        return CreateConstrainedHandle(handleName, target, bPosControl, bRotControl, bCreateControls)

    def CreateOffsetHandle(self, handleName, target, offset, bPosControl=True, bRotControl=True, refSpaceDag=None,
                           bCreateControls=True):
        return CreateOffsetHandle(handleName, target, offset, bPosControl, bRotControl, refSpaceDag, bCreateControls)

    def SelectDagList(self, dagList):
        return SelectDagList(dagList)

    def Parent(self, child, parent, nLogMode=vs.REPARENT_LOGS_OVERWRITE, bMaintainWorldPos=True):
        return Parent(child, parent, nLogMode, bMaintainWorldPos)

    def ParentMaintainWorld(self, child, parent):
        return ParentMaintainWorld(child, parent)

    def CreateHandleAt(self, handleName, target, bPosControl=True, bRotControl=True):
        return CreateHandleAt(handleName, target, bPosControl, bRotControl)

    def CreatePointOrientConstraint(self, target, slave, bCreateControls=True, group=None):
        return CreatePointOrientConstraint(target, slave, bCreateControls, group)

    def BuildArmLeg(self, rigPVTarget, rigEndTarget, bipStart, bipEnd, constrainEnd, group=None):
        return BuildArmLeg(rigPVTarget, rigEndTarget, bipStart, bipEnd, constrainEnd, group)

    def CreateAttachmentHandleInGroup(self, attachmentName, controlGroup):
        return CreateAttachmentHandleInGroup(attachmentName, controlGroup)

    def AddDagControlsToGroup(self, group, *dagList):
        return AddDagControlsToGroup(group, *dagList)

    def MoveControlGroup(self, groupName, oldParent, newParent):
        return MoveControlGroup(groupName, oldParent, newParent)

    def FindChildControlGroup(self, controlGroup, childName, bRecursive=False):
        return FindChildControlGroup(controlGroup, childName, bRecursive)
