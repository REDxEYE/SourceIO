...
def get_name(mdl_file: BinaryIO):
    if hasattr(mdl_file, 'name'):
        model_name = path_stem(mdl_file.name) + '_MODEL'
    else:
        model_name = 'GoldSrc_v4_MODEL'
    return model_name


def create_armature(model_name: str, mdl: Mdl, scale):
...
def import_model(name: str, mdl_buffer: Buffer, options: ModelOptions):
    ...
    for model in mdl.models:
        ...
        for model_index, mesh in enumerate(model.meshes):
            ...
            for index in range(0, len(mesh_triverts), 3):
                ...
                model_normals.extend((model.normals[v0.normal_index],
                                      model.normals[v1.normal_index],
                                      model.normals[v2.normal_index]))
                uv_per_mesh.append({
...
def load_material(model_name, texture_id, model_texture_info: StudioTexture, model_object):
    material = get_or_create_material(f'{model_name}_texture_{texture_id}', f'{model_name}_texture_{texture_id}')
